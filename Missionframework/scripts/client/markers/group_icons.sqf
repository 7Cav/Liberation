...
while { true } do {
    {
        if ((_x isNotEqualTo group player) && ((side _x isEqualTo GRLIB_side_friendly))) then {
            if ( (_x in _iconed_groups) && (
                (count units _x isEqualTo 0) ||  (side _x isEqualTo GRLIB_side_friendly && (((leader _x) distance (markerPos GRLIB_respawn_marker) < 100) || ((leader _x) distance startbase < 500))))) then {
                clearGroupIcons _x;
                _iconed_groups = _iconed_groups - [_x];
            };

            if ( !(_x in _iconed_groups) && (
                (count units _x > 0) &&  (side _x isEqualTo GRLIB_side_friendly && (((leader _x) distance (markerPos GRLIB_respawn_marker) > 100) && ((leader _x) distance startbase > 500))))) then {
                clearGroupIcons _x;
                _localgroup = _x;
                ...
                if ( side _localgroup isEqualTo GRLIB_side_friendly ) then {
