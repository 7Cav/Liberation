...
if ((alive _transVeh) && (alive (driver _transVeh))) then {
    ...
    while {(count (waypoints _infGrp)) isNotEqualTo 0} do {deleteWaypoint ((waypoints _infGrp) select 0);};
    ...
    while {(count (waypoints _transGrp)) isNotEqualTo 0} do {deleteWaypoint ((waypoints _transGrp) select 0);};
