...
while {true} do {
    ...
    _kp_markers_array = [];
    {
        if ((side _x isEqualTo GRLIB_side_enemy) && (((units _x) findIf {!captive _x} ) > 0) && ([(getpos leader _x), GRLIB_side_friendly, GRLIB_radiotower_size] call KPLIB_fnc_getNearestTower isNotEqualTo "")) then {
            ...
            if ((markerShape _kp_markers_name) == "RECTANGLE") then {
